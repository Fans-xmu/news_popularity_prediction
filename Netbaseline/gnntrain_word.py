...
#最后对所有节点做一个pooling或者attention得到该新闻的representation，
#对news有顺序的输入到一个GRU得到news的representation.
...
def main():
    ...
    # Important settings
    parser.add_argument('--model', type=str, default='WGcn', help='model structure[HSG|HDSG]')
    ...
    train_dataset = ExamplewordgcnSet(data_file, vocab, hps.doc_max_timesteps, hps.sent_max_len, train_w2s_path)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=hps.batch_size, shuffle=False,
                                               collate_fn=graph_collate_fn)
    del train_dataset
    valid_dataset = ExamplewordgcnSet(valid_file, vocab, hps.doc_max_timesteps, hps.sent_max_len,val_w2s_path)
    ...
    #     model = WGcn(hps, embed)
    #     logger.info("[MODEL] WGcn ")
    # elif hps.model == "FHgnn_allno":
    #      model = FHgnn_allno(hps, embed)
    #      logger.info("[MODEL]FHgnn_allno ")
