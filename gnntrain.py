...
import numpy as np
import torch
from Netbaseline.WGcn import WGcn
from FHgnn_noentity import FHgnn_noentity
from FHgnn_allno import FHgnn_allno
...
def setup_training_MAE(model, train_loader, valid_loader, valset, hps):
    ...
    train_loss_list = []
    val_loss_list = []
    train_dir = os.path.join(hps.save_root, "GNN")
...
def run_training_MAE(model, train_loader, valid_loader, valset, hps, train_dir, train_loss_list, val_loss_list):
    ...
    for epoch in range(1, hps.n_epochs + 1):
        ...
        for i, (G, index) in enumerate(train_loader):
            ...
            outputs = model.forward(G)  # [n_snodes, 1]
        ...
        if not best_train_loss or epoch_avg_loss < best_train_loss:
            save_file = os.path.join(train_dir, "bestmodel_weibo_MAE"+'_'+hps.model+'_'+str(hps.lstm_layers))
...
#---------------------------------------mind----------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='Hgnn Model')
    ...
    # Important settings
    parser.add_argument('--model', type=str, default='FHgnn_noentity', help='model structure[HSG|HDSG]')
    ...
    # Hyperparameters
    parser.add_argument('--gpu', type=str, default='2', help='GPU ID to use. [default: 0]')
    parser.add_argument('--cuda', action='store_true', default=True, help='GPU or CPU [default: False]')
    parser.add_argument('--vocab_size', type=int, default=60000, help='Size of vocabulary. [default: 60000]')
    # parser.add_argument('--vocab_size', type=int, default=50000,help='Size of vocabulary. [default: 50000]')
    parser.add_argument('--n_epochs', type=int, default=30, help='Number of epochs [default: 20]')
    ...
    train_dataset = ExamplegnnSet(data_file, vocab, hps.doc_max_timesteps, hps.sent_max_len, train_w2s_path)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=hps.batch_size, shuffle=False,
                                               collate_fn=graph_collate_fn)
    del train_dataset
    valid_dataset = ExamplegnnSet(valid_file, vocab, hps.doc_max_timesteps, hps.sent_max_len,val_w2s_path)
    valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=hps.batch_size, shuffle=False,
                                               collate_fn=graph_collate_fn)

    if hps.model == "FHgnn_noentity":
        model = FHgnn_noentity(hps, embed)
        logger.info("[MODEL] FHgnn_noentity ")
    elif hps.model == "WGcn":
        model = WGcn(hps, embed)
        logger.info("[MODEL] WGcn ")
    ...
    #                                           collate_fn=graph_collate_fn)
    # new_model = torch.load('./save/GNN/bestmodel_weibo_MAE'+'_'+hps.model+'_'+str(hps.lstm_layers))
