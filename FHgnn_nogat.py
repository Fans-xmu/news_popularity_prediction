...
class FHgnn_nogat(nn.Module):
    ...
    def __init__(self, hps, embed,entity_news_dict):
        ...
        # 改成预测数值
        self.wh = nn.Linear(self.feat_embed_size, 1)
    ...
    def forward(self, graph ,entity_map):
        ...
        :param graph: [batch_size] * DGLGraph
            ...
            news:
                sent2news: avg pooling
            entity_map:[entity_num,seqlen]
    ...
    def changenewsembed(self,graph,entity_result,_entity_news_dict,entity_map,newsid2nid):
        entity_result_list=entity_result.tolist()
        entity_map_list = entity_map.tolist()
        ##此处可拓展为attention
        for i in range(len(entity_map_list)):
