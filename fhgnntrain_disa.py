...
def setup_training_MAE(model, train_loader, valid_loader, valset, hps,entity_dict,vocab):
    ...
    train_loss_list = []
    val_loss_list = []
    train_dir = os.path.join(hps.save_root, "HGNN_nogat")
    ...
    try:
        ...
        save_model(model, os.path.join(train_dir, "earlystop"+'_'+hps.model))
...
def main():
    ...
    # Important settings
    parser.add_argument('--model', type=str, default='FHgnn_nogat', help='model structure[HSG|HDSG]')
    ...
    #                                           collate_fn=graph_collate_fn)
    # new_model = torch.load('./save/HGNN/bestmodel_weibo_MAE'+'_'+hps.model)
